uint64_t sym__fnv1a32(uint64_t X0_0, uint64_t X1_0)
{
    /* r2dec proof: no individual construct is marked; 40 source obligations: 30 rendered, 10 elided, 0 refused; 22 statements rendered */
    {
        uint8_t* X8_1;
        uint64_t X0_2;
        uint64_t X1_1;
        X8_1 = (uint8_t*)X0_0;
        X0_2 = (uint64_t)0x811c9dc5;
        X1_1 = X1_0;
        if (X1_0 != 0) {
            uint32_t tmp_2a000_4 = 0x1000193;
            for (; ; ) {
                uint8_t* tmp_7400_2 = X8_1;
                X8_1 = (uint8_t*)((uint64_t)X8_1 + 1);
                uint8_t tmp_25400_2 = *tmp_7400_2;
                uint32_t tmp_20380_2 = (uint32_t)X0_2 ^ (uint32_t)tmp_25400_2;
                X0_2 = (uint64_t)(uint32_t)(tmp_20380_2 * tmp_2a000_4);
                uint64_t tmp_3e280_2 = X1_1 - 1;
                uint8_t TMPZR_2 = X1_1 == 1;
                uint64_t X1_2 = tmp_3e280_2;
                uint8_t tmp_a00_2 = !TMPZR_2;
                X1_1 = X1_2;
                if (!tmp_a00_2) {
                    break;
                }
            }
        }
        return X0_2;
    }
}

