...
# then continues. For an interactive session use the lldb MCP server instead
# (tests/corpus/lldb_mcp.sh, configured in .mcp.json) after
# tests/corpus/probe_plugin.sh install. R2_BIN names the radare2 (default: the fork's build tree),
...
"$root/tests/corpus/probe_plugin.sh" install >&2
trap '"$root/tests/corpus/probe_plugin.sh" restore >/dev/null' EXIT
...
trap '"$root/tests/corpus/probe_plugin.sh" restore >/dev/null; rm -f "$commands"' EXIT
{
