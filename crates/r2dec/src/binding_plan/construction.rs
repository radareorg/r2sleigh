...
fn apply_parameter_declaration_types(
    ...
    for (slot, disposition) in parameters.iter().enumerate() {
        ...
        // The one declaration r2types states for this slot, where it describes the binding's width.
        if let Some(ty) = source_owned.parameter_declaration(slot, binding_width_bits) {
            binding.declaration_type = ty;
        }
    }
}
...
impl BindingPlan {
    ...
    pub(crate) fn build_shadow_with_control(
        ...
        let elision_for =
            |value: r2ssa::ValueId, dead: crate::ledger::ElisionReason| match effectful
                .contains(&value)
            {
                true => crate::ledger::ElisionReason::UnreadEffectfulValue,
                false => dead,
            };
        ...
        for (index, graph_value) in graph.values.iter().enumerate() {
            ...
            if return_controls.contains(&graph_value.id) {
                ...
                dispositions[index] = ValueDisposition::Elided {
                    reason: crate::ledger::ElisionReason::CallReturnAddress,
                    proof: ValueElisionProof {
                        authority: source.authority().clone(),
                        value: graph_value.id,
                    },
